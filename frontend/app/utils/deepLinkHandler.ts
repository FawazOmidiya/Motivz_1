...
export function handleDeepLink(url: string): boolean {
  ...
  try {
    switch (deepLinkData.type) {
      case "event":
        router.push(`/screens/EventDetail?id=${deepLinkData.id}`);
        return true;

      case "club":
        router.push(`/screens/ClubDetail?id=${deepLinkData.id}`);
        return true;

      case "user":
        router.push(`/screens/UserProfileScreen?userId=${deepLinkData.id}`);
        return true;
