...
export async function POST(
  request: NextRequest
): Promise<NextResponse<RecurringEventGenerationResult>> {
  try {
    ...
    return NextResponse.json({
      message: dry_run
        ? "Dry run completed"
        : "Recurring events generated successfully",
...
async function generateRecurringInstances(
  ...
  while (
    currentDate <= endGenerationDate &&
    instanceCount < (config.max_occurrences || 12)
  ) {
